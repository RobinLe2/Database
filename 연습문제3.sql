...
/*
    ...
    158
*/
SELECT COUNT(first_name) AS five_letter_names
FROM customer
...
FROM rental 
WHERE return_date - rental_date  >=10;
...
/*
    ...
    599
*/
SELECT COUNT(customer_id) AS feb_2006_customer
FROM customer
...
/*
    ...
    G       41
*/
SELECT rating , COUNT(length) AS film_count
FROM film
WHERE length >= 150
GROUP BY rating;
...
RIGHT OUTER JOIN film f ON  i.film_id = f.film_id;
...
FROM film f
JOIN film_actor Rel ON f.film_id=Rel.film_id
JOIN actor a ON Rel.actor_id = a.actor_id
GROUP BY title;
...
JOIN country nation ON ci.country_id = nation.country_id
...
/*
    ...
    Travel    113.3158
*/
SELECT c.name AS name, AVG(length) AS avg_length
FROM category c
...
SELECT first_name, last_name,  (SELECT COUNT(*) 
        FROM rental 
        WHERE customer.customer_id = rental.customer_id) AS rental_count
FROM customer 
